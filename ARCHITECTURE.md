...
Le système est conçu comme une suite d'outils modulaires orchestrés autour d'un pipeline de données centralisé (`datav1/`). Il combine l'extraction de métriques Oracle, leur analyse via des modèles de langage (LLM) enrichis par une base de connaissances (RAG), et une restitution via une interface Web.
...
graph TD
    ...
    Extractor -->|CSV Files| DataStore[Dossier datav1/]
...
- **LLM Engine** : Interface vers l'API DeepSeek pour l'analyse générative. Gère les prompts et le contexte système.
...
## Structure des Données (`datav1/`)
Le dossier `datav1/` sert d'échangeur de données :
